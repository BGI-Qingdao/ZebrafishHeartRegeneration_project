...
scRNA <- readRDS('./scRNA.H_atrium.rds')
scRNA <- subset(scRNA,subset =  celltype != "Endothelial cells")
scRNA <- subset(scRNA,subset =  celltype != "Lymphatic Ecs")
...
for (index in 1:200){
    ...
    if ( file.exists(h5path) ) {
        ...
        if ( file.exists(rest))
            next
        if (h5path ==  "./00.data/s26.h5ad")
            next
        if (h5path ==  "./00.data/s120.h5ad")
            next
        if (h5path ==  "./00.data/s123.h5ad")
            next
        if (h5path ==  "./00.data/s126.h5ad")
            next
